...
class User {
    ...
    init(username: String, passwordHash: Int, name: String, phone: String, bio: String, image: UIImage) {
            ...
            if user.username == Username.text {
                if user.passwordHash == Password.text?.hashValue {
                    ...
                    if let vc = storyboard.instantiateViewController(withIdentifier: "ProfileViewController") as? ProfileViewController {
                        vc.user = user
                        navigationController?.pushViewController(vc, animated: true) //make changes here
                    }
                }
                else {
